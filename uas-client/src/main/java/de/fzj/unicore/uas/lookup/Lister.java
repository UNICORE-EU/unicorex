 ...
 * @author schuller
 */
public class Lister<T extends BaseWSRFClient> implements Iterable<T>{
	 ...
	 * Create a lister
	 * 
	 * @param executor - the executor service to use. If <code>null</code>, lookup 
	 * will be done synchronously on the calling thread.
	...
	public Lister(ExecutorService executor, AddressFilter<T> filter, int maxQueueSize){
		...
		this.addressFilter=filter!=null?filter:new AcceptAllFilter<T>();
	...
	/**
	 * Get an iterator to access the loopup results. If not already done using 
	...
	public Iterator<T> iterator(){
		...
		return new Iterator<T>() {
			...
			public boolean hasNext() {
				return (running==true) && (runCounter.get()>0 || queue.size()>0);
			}
	...
	public static class AcceptAllFilter<T> implements AddressFilter<T> {
